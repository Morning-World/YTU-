...
public class ChangeGrade extends JFrame implements ActionListener{
    ...
    public static void main(String[] args[]){
        ChangeGrade cg = new ChangeGrade("20181001", "C语言");
    }
        ...
        if(new_grade.isEmpty()){
            ...
            try{
                ...
                if(subject.equals("高数")) {
                    sql = "update Score set 高数=? where 学号=?";
                    ps = ct.prepareStatement(sql);
                }else if((subject.equals("C语言"))) {
                    sql = "update Score set C语言 = ? where 学号=?";
                    ps = ct.prepareStatement(sql);
                }else if(subject.equals("Java")) {
                    sql = "update Score set java=? where 学号=?";
                    ps = ct.prepareStatement(sql);
                } else if(subject.equals("英语")){
                    sql = "update Score set 英语=? where 学号=?";
                    ps = ct.prepareStatement(sql);
                }
                ps = ct.prepareStatement(sql); // Prepare the statement outside switch (assuming sql is set correctly inside switch)
